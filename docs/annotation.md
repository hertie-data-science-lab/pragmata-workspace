...
2. **Calibration agreement** - Krippendorff α over the calibration overlap. The headline is **pooled**: item-level data from every domain goes into one reliability matrix per (task, label), and those α are averaged unweighted across labels to give a figure per task. α is a ratio (1 − Do/De), and ratios do not average: the mean of per-domain α is not the α of the pooled data. Per-domain values stay a diagnostic in the report's collapsed breakdown. Each pooled α is published with the marginals behind it (ratings, minority-class count, prevalence, De), because a label whose minority class is small has De ≈ 0 and an unstable α; a degenerate one is flagged ⚠ in the table, and why that flag matters is in [Report deliverables](report-deliverables.md#agreement-annotation_label_summarycsv).
