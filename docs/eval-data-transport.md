...
| `predictions/` | GPU → CPU | per-row model predictions from an eval run - the whole `data/eval/prediction_outputs/` tree, one directory per (evaluator, population) |
...
`publikationsbot/` runs CPU → GPU as well, for the curated corpus the `corpus` prediction
population is staged from (`make transfer-pull PREFIX=publikationsbot`); it is not part of the
frozen export, so it travels on its own.
...
**A pulled tree cannot be consumed where it lands.** `pull` writes only under
`data/transfer/`, while pragmata resolves evaluator runs and `--prediction-id` under
`data/eval/`, so `predictions/` and `checkpoints/` have to be copied across after verifying -
see [Eval prediction](eval-prediction.md#getting-the-data-in-and-out). The export prefixes do
not need this: the scripts that read them fall back to the `data/transfer/` copy themselves.
