...
The transport is the [Blob pipe](data-transport.md); this stage adds one prefix, `publikationsbot/`. Do this before the run order below - the corpus population will not stage without it.
...
make eval-predict-inputs POPULATION=corpus             # pull the curated corpus first (above)
