...
- `corpus` - corpus scale, no human baseline at all. Read only with the evaluator's own test
  ...
  are directional at best (see docs/eval-synthetic-evaluator.md).
...
def unscoreable_labels(prediction_dir: Path, task: str) -> list[str]:
    ...
    see docs/eval-synthetic-evaluator.md - so its predictions.csv has three label columns. pragmata's
