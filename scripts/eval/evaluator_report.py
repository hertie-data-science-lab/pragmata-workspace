...
`evaluator_calibration.csv`). These describe the EVALUATORS, not the population they are
applied to: those estimates are `eval_metric_estimates.csv` (human labels) and
`synthetic_metric_estimates.csv` (the evaluators applied to a population). Read those two
beside this one - a population rate from a model with an AUC near chance is not a measurement.
