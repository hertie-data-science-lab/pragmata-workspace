...
The executable half of ``docs/data-dictionary.md``, which defines response / record /
...
# writes scores/ - and the ownership rule in docs/report-deliverables.md is that this tree holds only what
...
def _freeze_pin(key: str) -> str:
    ...
    if not FREEZE_CONF.exists():
        raise SystemExit(
            ...
            "  the file from git. See docs/report-deliverables.md."
        )
    ...
    try:
        ...
        try:
            ...
            raise SystemExit(
                ...
                "  configs/eval/train-requirements.txt instead - see docs/synthetic-evaluators.md."
            ) from exc
...
def require_gpu(*, use_cpu: bool = False) -> None:
    ...
    raise SystemExit(
        ...
        "  container first. See docs/synthetic-evaluators.md."
    )
