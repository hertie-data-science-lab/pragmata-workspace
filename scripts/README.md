...
- `eval/` - the eval-stage report scripts (the deliverable CSVs), plus the model stage: `train_evaluators.py` (train the synthetic evaluators), `predict_evaluators.py` (stage unlabelled populations and apply an evaluator), `score_synthetic_predictions.py` and `evaluator_report.py` - all four are documented together in [Synthetic evaluators](../docs/synthetic-evaluators.md). Plus `eval_common.py` - the shared vocabulary, and the pragmata/GPU/evaluator-run resolution the four model-stage scripts have in common.
