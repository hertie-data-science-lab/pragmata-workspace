...
def _html_table(headers: list[str], rows: list[list]) -> str:
    """HTML table; each cell is a plain string, or an (html, attr) tuple when it needs
    inline styling — background shading / right alignment — that a markdown pipe-table
    can't express. Plain tables use _table(); reserve this for shaded ones."""
    def td(cell) -> str:
        text, attr = cell if isinstance(cell, tuple) else (cell, "")
        return f"<td{attr}>{text}</td>"

    head = "".join(f"<th>{h}</th>" for h in headers)
    body = "\n".join("<tr>" + "".join(td(c) for c in row) + "</tr>" for row in rows)
...
def iaa_by_domain(domains: dict, total: dict) -> str:
    ...
    for name, v in sorted(domains.items(), key=alpha_key):
        ...
        if not a.get("n_labels"):
            continue
        rows.append([name, _alpha_cell(a.get("mean_alpha")), (str(a["n_labels"]), _RIGHT)])
    ta = total.get("agreement") or {}
    if ta.get("n_labels"):
        tma = ta.get("mean_alpha")
        rows.append([("<b>TOTAL</b>", ""), (f"<b>{_alpha(tma)}</b>", _alpha_shade(tma)),
    ...
    for name, v in sorted(domains.items()):
        for task in TASK_ORDER:
            ...
            for lbl, lv in per_label.items():
                rows.append([name, task, lbl, _alpha_cell(lv["alpha"]),
                             (_pct(lv["pct_agreement"]), _RIGHT), (_prev(lab.get(lbl)), _RIGHT),
                             (_int(lv["n_items"]), _RIGHT), (str(lv["n_annotators"]), _RIGHT)])
    if not rows:
        return ""
    return _html_table(["Domain", "Task", "Label", "α", "% agree", "Prev.*", "Items†", "Ann."], rows)


def _label_rows(per_label: dict) -> list[list[str]]:
    # Both numerator (# true) and denominator (n); prevalence = # true / n.
    return [[lbl, _int(lv["n_true"]), _int(lv["n"]), _prev(lv)]
            for lbl, lv in per_label.items() if lv["n"] > 0]
...
def label_distribution_totals(total: dict) -> str:
    ...
    if not rows:
        return ""
    return _table(["Task", "Label", "# true", "n", "Prev.*"], ["l", "l", "r", "r", "r"], rows)
...
def label_distribution_by_domain(domains: dict) -> str:
    ...
    if not rows:
        return ""
    return _table(["Domain", "Task", "Label", "# true", "n", "Prev.*"],
                  ["l", "l", "l", "r", "r", "r"], rows)
...
def _bias_by_annotator(domains: dict) -> dict[str, list[dict]]:
    ...
    for name, v in sorted(domains.items()):
        for task in TASK_ORDER:
            ...
            if len(by_ann) < 2:
                continue
            pool: dict[str, int] = {}  # label -> # annotators who gave it a non-null value
            ...
            for uuid, labs in by_ann.items():
                for lbl, lv in labs.items():
                    ...
                    by_uuid.setdefault(uuid, []).append(
                        {"delta": d, "n": lv["n"], "pool": pool.get(lbl, 0), "domain": name,
                         "task": task, "label": lbl, "prevalence": lv["prevalence"]})
    return by_uuid
...
def annotator_bias(domains: dict) -> str:
    ...
    for uuid in order:
        ...
        for i, r in enumerate(recs):
            ...
            delta_cell = (delta, _shaded(_RED_BG, right=True) if is_substantial(r) else _RIGHT)
            rows.append([_uid(uuid) if i == 0 else "", r["domain"], r["task"], r["label"],
                         (_pct(r["prevalence"]), _RIGHT), delta_cell, (_int(r["n"]), _RIGHT),
                         (str(r["pool"]), _RIGHT)])
    table = _html_table(["Annotator", "Domain", "Task", "Label", "Prev.*", "Δ pp", "n", "Pool"], rows)
    return f"{note}\n\n{table}"
...
def render(snap: dict) -> str:
    ...
    iaa_parts = []
    if bd or bt or by_label:
        iaa_parts.append(_note(
    ...
    labels_done = False
    if bd:
        iaa_parts += [f"### By domain\n\n{bd}", labels_note]  # ‡Labels first appears here
        labels_done = True
    if bt:
        iaa_parts.append(f"### By task\n\n{bt}")
        if not labels_done:
            iaa_parts.append(labels_note)
    if by_label:  # Prev.* and Items† first appear in this table
        iaa_parts += [
            _collapsible("Detailed breakdown by label", by_label),
            _note("**\\*Prev. = prevalence**: the share of submitted annotations where the "
    ...
    for title, body in [
        ...
        ("## Per-annotator label bias", annotator_bias(domains)),  # Δ/n/Pool note sits above its table
    ]:
        if body:
            parts.append(f"{title}\n\n{body}")
    parts += [
        "## Rate of annotation\n\n" + _collapsible("By annotator", per_annotator_timing(total)),
        _note("**Median gap** is a *constructed* proxy for pace, not a direct measure of effort. "
              ...
              "Lower = faster; **Pace** is its inverse (records/hour)."),
        "### Domain-level pace\n\n" + domain_pace(domains),
