...
def cmd_restore(backup_dir: str, target_workspace: str | None) -> None:
    ...
    where = (
        f"workspace {target_workspace!r}"
        if target_workspace
        else "their original workspaces"
    )
    ...
    for m in dsets:
        ...
        ds = rg.Dataset(
            name=m["name"], workspace=wspace, settings=settings, client=client
        ).create()
        assert ds.workspace.name == ws_name, (
            f"landed in {ds.workspace.name!r}, not {ws_name!r}"
        )
        records = [
            deserialize_record(d)
            for d in json.loads((Path(m["path"]) / "records_full.json").read_text())
        ]
        ds.records.log(records)
        print(
            f"  [restored] {ws_name}/{m['name']}  ({len(records)} records, "
...
def main() -> None:
    ...
    r = sub.add_parser(
        "restore", help="recreate datasets from a dump (status-preserving)"
    )
