...
(gitignored), a `{username: password}` map that `setup.sh` merges into the
roster at runtime before calling pragmata. A user absent from the overlay gets
a password auto-generated by pragmata and printed once at setup. `setup` is
idempotent: existing Argilla accounts are skipped, so re-running never changes
a live user's password.
